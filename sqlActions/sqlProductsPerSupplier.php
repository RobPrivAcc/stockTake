  ...
  FROM [Stock] WHERE Discontinued = '0' AND SupplierName = '$supplierList';";
    ...
    $div = "<DIV class = 'row'>";
		$div .= "<DIV class = 'col-xs-12 col-s-12 col-12'>";
			$div .= "Calculation taken up to: ".date('Y-m-d', strtotime('-1 month'));
		$div .= "</DIV>";
	$div .= "</DIV>";
	$div .= "<DIV class = 'row'>";
    ...
    for($i=0; $row = $query->fetch(); $i++){
        ...
        $packSize = $row['PackSize'];
				$div .= "<DIV class = 'col-s-1 col-1'>";
					$div .= "<input type='checkbox' id='subscribeNews_".$i."' name='subscribe' value='newsletter'>";
				$div .= "</DIV>";
        ...
        $div .= "<DIV class = 'col-xs-12 col-s-12 col-12' style = 'border-bottom: 1px solid black;'>";
			...
			$div .= "<DIV class = 'row'>";
                ...
                $div .= "<DIV class = 'col-s-12 col-12'><strong>";
                    $div .= $row['Name of Item']."  (".round($packSize,0).")    Weakly avarage(".($minMax->getMonth($productName)/4).")";
                $div .= "</strong></DIV>";
            $div .= "</DIV>";
    ...
    $div .="</DIV>";
        $div .= "<DIV class = 'row'>";
        $div .= "<DIV class = 'col-s-4 col-4'>";
            //print_r(json_encode($updateArray));
            $div .="<input type ='hidden' id='minMaxArray' value='".json_encode($updateArray)."'/>";
			//$div .="<input type ='hidden' id='minMaxArray' value='oijhh'/>";
